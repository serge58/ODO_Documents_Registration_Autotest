...
import tests.run.outgoing.testitself.odo51_test;

@Listeners({ TestListener.class })

public class odo51_EditDocDraft {
static WebDriver driver;
	...
	@Test(description = "Принимаем к учёту новый документ", dataProvider = "outgoingDocDraft", dataProviderClass = ODODataProviders.class, dependsOnMethods = { "Autorisation" })
	public void EditDocDraft(HashMap<String, String> docDraftFields) {
		odo51_test.EditDocDraft(docDraftFields);
	}
	
	@Test(description = "Проверяем все поля карточки после регистрации документа", dependsOnMethods = { "EditDocDraft" })
	public void CheckDocDraft() {
		odo51_test.CheckDocDraft();
	}
